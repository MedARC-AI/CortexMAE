...
overrides="model_kwargs.ckpt_path=${ckpt_path} steps_per_epoch=200 epochs=20 warmup_epochs=5 cv_metric=acc batch_size=${bs} accum_iter=2 lr=0.001 num_workers=8 wandb=false"

notes="input_space ablation v2 $key; eval (${dataset} ${repr} ${clf}); modified probe config v2, dense lr grid 20 epochs"

name="${EXP_NAME}/${key}/eval_2/${dataset}__${repr}__${clf}"
